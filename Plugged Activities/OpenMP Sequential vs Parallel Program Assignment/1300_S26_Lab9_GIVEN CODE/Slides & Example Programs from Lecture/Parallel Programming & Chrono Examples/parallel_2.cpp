...
int main() {
    ...
    #pragma omp parallel
    {
		...
		// which means this code can only be executed by one thread at a time. 
		// All other threads trying to enter this "critical" region must wait 
		// until the thread inside finishes. This prevents race conditions 
		// when multiple threads try to update or print to the same resource (like cout)
